			...
			<div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
				<div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
					<h6 class="text-white text-capitalize ps-3">
						<spring:message code="threadCategories.title"/>
					</h6>
					<a class="btn bg-gradient-dark mb-0 mx-2" href="<c:url value='/thread-categories/create' />">
						<i class="material-icons text-sm">add</i>&nbsp;&nbsp;
						<spring:message code="add.new.threadCategory"/>
					</a>
				</div>
			</div>
			<div class="card-body px-0 pb-2">
				<!-- Search Form -->
				<form action="<c:url value='/thread-categories' />" method="get" class="mx-4">
					<div class="row">
						...
						<div class="col-md-2 mb-3">
							<div class="input-group input-group-outline">
								<select name="forum" id="forum" class="form-control">
									<option value=""><spring:message code="all.threadCategory.forum"/></option>
									<c:forEach items="${forums}" var="f">
										<option value="${f.id}" ${forum == f.id ? 'selected' : ''}>
												${f.name}
										</option>
									</c:forEach>
								</select>
				...
				<!-- Bulk Action Form -->
				<form id="bulkActionForm" action="<c:url value='/thread-categories/bulk-action' />" method="post">
					...
					<div class="table-responsive p-0">
						<table class="table align-items-center mb-0">
							...
							<tr>
								...
								<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-3">
									<a href="<c:url value='/thread-categories?sort=id&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
								...
								<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
									<a href="<c:url value='/thread-categories?sort=name&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
										<spring:message code="name"/>
										<i class="material-icons">${param.sort eq 'name' && param.direction eq 'asc' ? 'arrow_upward' : 'arrow_downward'}</i>
									</a>
								</th>
								<th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
									<a href="<c:url value='/thread-categories?sort=forumCategory&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
										<spring:message code="forumCategory"/>
										<i class="material-icons">${param.sort eq 'forumCategory' && param.direction eq 'asc' ? 'arrow_upward' : 'arrow_downward'}</i>
									</a>
								</th>
							...
							<tbody>
							<c:forEach items="${threadCategories}" var="threadCategory">
								<tr>
									<td class="ps-3">
										<input type="checkbox" name="selectedIds" value="${threadCategory.id}"
											   class="selectItem">
									</td>
									<td class="ps-3">
										<p class="text-xs font-weight-bold mb-0">${threadCategory.id}</p>
									</td>
									<td>
										<p class="text-xs font-weight-bold mb-0">${threadCategory.name}</p>
									</td>
									<td>
										<p class="text-xs font-weight-bold mb-0">${threadCategory.forumCategory.name}</p>
									</td>
									<td class="align-middle">
										<a href="<c:url value='/thread-categories/${threadCategory.id}/update' />"
										   class="text-dark font-weight-bold text-xs" data-toggle="tooltip"
										   data-original-title="<spring:message code='edit' />">
											<spring:message code="edit"/>
										</a>
										|
										<a href="<c:url value='/thread-categories/${threadCategory.id}/delete' />"
