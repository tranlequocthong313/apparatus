            ...
            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                    <h6 class="text-white text-capitalize ps-3">
                        <spring:message code="providers.title"/>
                    </h6>
                    <a class="btn bg-gradient-dark mb-0 mx-2" href="<c:url value='/providers/create' />">
                        <i class="material-icons text-sm">add</i>&nbsp;&nbsp;
                        <spring:message code="add.new.provider"/>
                    </a>
                </div>
            </div>
            <div class="card-body px-0 pb-2">
                <!-- Search Form -->
                <form action="<c:url value='/providers' />" method="get" class="mx-4">
                    <div class="row">
                ...
                <!-- Bulk Action Form -->
                <form id="bulkActionForm" action="<c:url value='/providers/bulk-action' />" method="post">
                    ...
                    <div class="table-responsive p-0">
                        <table class="table align-items-center mb-0">
                            ...
                            <tr>
                                ...
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-3">
                                    <a href="<c:url value='/providers?sort=id&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
                                ...
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7">
                                    <a href="<c:url value='/providers?sort=name&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
                                        <spring:message code="name"/>
                                        <i class="material-icons">${param.sort eq 'name' && param.direction eq 'asc' ? 'arrow_upward' : 'arrow_downward'}</i>
                                    </a>
                                </th>
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                    <a href="<c:url value='/providers?sort=phoneNumber&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
                                ...
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                    <a href="<c:url value='/providers?sort=email&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
                                        <spring:message code="email"/>
                                        <i class="material-icons">${param.sort eq 'email' && param.direction eq 'asc' ? 'arrow_upward' : 'arrow_downward'}</i>
                                    </a>
                                </th>
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                    <a href="<c:url value='/providers?sort=website&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
                                        <spring:message code="website"/>
                                        <i class="material-icons">${param.sort eq 'website' && param.direction eq 'asc' ? 'arrow_upward' : 'arrow_downward'}</i>
                                    </a>
                                </th>
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                    <a href="<c:url value='/providers?sort=address&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
                                ...
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                    <a href="<c:url value='/providers?sort=createdAt&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
                                ...
                                <th class="text-uppercase text-secondary text-xxs font-weight-bolder opacity-7 ps-2">
                                    <a href="<c:url value='/providers?sort=updatedAt&direction=${param.direction eq \'asc\' ? \'desc\' : \'asc\'}' />">
                            ...
                            <c:forEach items="${providers}" var="provider">
                                <tr>
                                    <td class="ps-3">
                                        <input type="checkbox" name="selectedIds" value="${provider.id}"
                                               class="selectItem">
                                    </td>
                                    <td class="ps-3">
                                    ...
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">${provider.phoneNumber}</p>
                                    </td>
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">${provider.email}</p>
                                    </td>
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0 text-decoration-underline text-primary">
                                            <a class="text-primary" href="${provider.website}" target="_blank"><spring:message code="website.link" /></a>
                                        </p>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">${provider.createdAt}</p>
                                    </td>
                                    <td>
                                        <p class="text-xs font-weight-bold mb-0">${provider.updatedAt}</p>
                                    </td>
                                    <td class="align-middle">
                                        <a href="<c:url value='/providers/${provider.id}/update' />"
                                           class="text-dark font-weight-bold text-xs" data-toggle="tooltip"
                                           data-original-title="<spring:message code='edit' />">
                                            <spring:message code="edit"/>
                                        </a>
                                        |
                                        <a href="<c:url value='/providers/${provider.id}/delete' />"
